...
test_that("isativ() works correctly with argument 'fast'", {
  ...
  expect_error(ivisat(formula = fml, data = df, iis = TRUE,
                      print.searchinfo = FALSE, overid = 0.1, fast = TRUE))
  expect_error(ivisat(formula = fml, data = df, iis = TRUE,
                      print.searchinfo = FALSE, weak = 0.05, fast = TRUE))
  expect_error(ivisat(formula = fml, data = df, iis = TRUE,
                      print.searchinfo = FALSE, overid = 0.05, weak = 0.05,
                      fast = TRUE))
  ...
  m2 <- ivisat(formula = fml, data = df, iis = FALSE, sis = TRUE,
               print.searchinfo = FALSE, fast = TRUE) # nothing retained
  expect_type(m2, "list")
  expect_length(m2, 2L)
  expect_named(m2, c("selection", "final"))
  expect_identical(class(m2$final), "ivreg")
  ...
  m3 <- ivisat(formula = fml, data = df, iis = FALSE, sis = FALSE, tis = TRUE,
               print.searchinfo = FALSE, fast = TRUE) # many retained
  expect_type(m3, "list")
  expect_length(m3, 2L)
  expect_named(m3, c("selection", "final"))
  expect_identical(class(m3$final), "ivreg")
  ...
  # check that output and selection is the same for fast == TRUE and fast == FALSE
  m1s <- ivisat(formula = fml, data = df, iis = TRUE, print.searchinfo = FALSE)
  m2s <- ivisat(formula = fml, data = df, iis = FALSE, sis = TRUE,
               print.searchinfo = FALSE)
  m3s <- ivisat(formula = fml, data = df, iis = FALSE, sis = FALSE, tis = TRUE,
                print.searchinfo = FALSE)
  m1s$selection$time.started <- NULL
  m1s$selection$time.finished <- NULL
  m1s$selection$date <- NULL
  m2s$selection$time.started <- NULL
  m2s$selection$time.finished <- NULL
  m2s$selection$date <- NULL
  m3s$selection$time.started <- NULL
  ...
  expect_identical(m3, m3s, ignore_attr = c("waldo_opts", ".Environment"))
