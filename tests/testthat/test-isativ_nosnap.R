...
test_that("ivisat() works correctly", {
  ...
  # check that get error when uislist but matrices without colnames
  colnames(uislist[[1]]) <- NULL
  expect_error(ivisat(formula = fml, data = df, iis = FALSE, print.searchinfo = FALSE,
                      uis = uislist),
               "The matrices in the list 'uis' all must have column names")
  ...
  # specify overid test when model not overid
  expect_warning(ivisat(formula = y ~ -1+cons+x1+x2 | -1+cons+x1+z2,
                        data = df, iis = TRUE, t.pval = 1/100, print.searchinfo = FALSE, overid = 0.05),
                 "Sargan test of overidentifying restrictions cannot be calculated")
...
test_that("isat.ivreg() works correctly", {
  ...
  expect_error(isat(base2, print.searchinfo = FALSE), "Please specify 'model = TRUE' in the original function call")

  base3 <- ivreg::ivreg(formula = fml, data = df, weights = c(rep(1/2, 25), rep(3/2, 25)))
  expect_error(isat(base3, print.searchinfo = FALSE), "GETS modelling currently does not support weights")

  class(base3) <- "ivregressions"
  expect_error(isat(base3))
